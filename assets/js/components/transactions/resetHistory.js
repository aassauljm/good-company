...
@connect((state, ownProps) => {
    ...
    return {
        resetAction: (args) => {
            return dispatch(updateResource(`/company/${ownProps.transactionViewData.companyId}/reparse_reset_pending_history`, {}, {loadingMessage: 'Reimporting Company'}))
            .then(() => {
                ownProps.end();
                dispatch(resetTransactionViews())
                dispatch(addNotification({message: 'Company Reconciliation Reset'}));
...
export class ReparseResetHistoryTransactionView extends React.Component {
    constructor(props) {
        super(props);
        this.handleReset = ::this.handleReset;
        this.handleClose = ::this.handleClose;
